...
class PaymentConnectorConfigurationService {
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Meeteeor\Sdk\Model\PaymentConnectorConfiguration',
				'/payment-connector-configuration/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Meeteeor\Sdk\Model\PaymentConnectorConfiguration', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Meeteeor\Sdk\Model\EntityQuery $query The query restricts the payment connector configuration which are returned by the search. (required)
	 * @throws \Meeteeor\Sdk\ApiException
	 * @throws \Meeteeor\Sdk\VersioningException
	 * @throws \Meeteeor\Sdk\Http\ConnectionException
	 * @return \Meeteeor\Sdk\Model\PaymentConnectorConfiguration[]
