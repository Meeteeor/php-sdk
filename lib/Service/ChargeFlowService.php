...
class ChargeFlowService {
	 ...
	 * @throws \Planet\Sdk\VersioningException
	 * @throws \Planet\Sdk\Http\ConnectionException
	 * @return \Planet\Sdk\Model\ChargeFlow
	 */
	...
	public function readWithHttpInfo($space_id, $id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\Planet\Sdk\Model\ChargeFlow',
				'/charge-flow/read'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\Planet\Sdk\Model\ChargeFlow', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
	 ...
	 *
	 * @param int $space_id  (required)
	 * @param \Planet\Sdk\Model\EntityQuery $query The query restricts the charge flows which are returned by the search. (required)
	 * @throws \Planet\Sdk\ApiException
	 * @throws \Planet\Sdk\VersioningException
	 * @throws \Planet\Sdk\Http\ConnectionException
	 * @return \Planet\Sdk\Model\ChargeFlow[]
